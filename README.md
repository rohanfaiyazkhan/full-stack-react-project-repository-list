...
I certainly spent a little longer than the recommended 3-4 hours as I ran into some issues with my development setup, was a little rusty, and over-engineered the frontend. The API route implemented is quite simple, makes a call to the Github API, sorts the results and sends. The web uses Tailwind for styles and implements a basic caching layer which uses loading state indicator and request timestamps (which has the capability to do offline caching and staleness verification eventually but I stopped myself before going that far). The commits are fetched directly from Github instead of making a round trip to the server though the latter is how I would have preferred to do it. Markdown is rendered with React Markdown.

Some screenshots are provided below. I had a lot of fun with this project and look forward to discussing my approach with you.
